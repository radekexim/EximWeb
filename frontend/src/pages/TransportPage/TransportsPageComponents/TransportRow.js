...
export default function TransportRow(props) {
    ...
    const getInformationOrdersByClient = (ordersTable) => {
        let newordersClient = [];
        const clientAddress = ordersTable.map((order) => order.addressdelivery).filter((v, i, a) => a.indexOf(v) === i);
    ...
    return (
        <>
            <TableRow key={row.id} sx={{ '& > *': { borderBottom: 'unset', backgroundColor: openOrders ? '#e1f5fe' : 'white' } }}>
                <TableCell>
                    <IconButton
