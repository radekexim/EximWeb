...
export default function Row(props) {
    ...
    return (
        <>
            <TableRow sx={{ '& > *': { borderBottom: 'unset', backgroundColor: openTransport ? '#90caf9' : 'white' } }}>
                <TableCell>
                    <IconButton
                ...
                <TableCell align="right">
                    {row.margin_delivery.toFixed(2)}
                </TableCell>
                <TableCell align="right">
                    <IconButton><EditIcon /></IconButton>
                    <DeleteTransportButton deletefunction={() => console.log('usunięte')} name={row.name}></DeleteTransportButton>
                    <IconButton onClick={() => props.printtable(row.id)}><PrintIcon /></IconButton>
                </TableCell>
            </TableRow>
            <TableRow>
