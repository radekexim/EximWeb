...
export const getInformationToPrint = async (transportData, id) => {
  ...
  try {
    const response = await axios.post('/getInformationTransport', {
      orders: ordersTable.transportOrders,
    })
    ...
    ordersTable.transportOrders.forEach((order) => {
      ...
      for (const key in constructions) {
        let countConstruction = newOrder
          .filter((el) => el.konstrukcja.includes(constructions[key].value))
          .reduce((sum, a) => sum + a.sztuki, 0)
        if (countConstruction !== 0) {
          projectString += countConstruction + ' ' + constructions[key].name + ' '
          projects.push({
            id: constructions[key].id,
            name: constructions[key].name,
            value: countConstruction,
          })
        }
      }
      ...
      newOrder.forEach((position) => {
        ...
        let name = constructions.filter((el) => el.value === position.konstrukcja.slice(0, 3))[0]
          .name
        if (
          position.progress < progressToFinish &&
          !position.konstrukcja.includes('109') &&
          !position.konstrukcja.includes('108')
        ) {
          notFinishConstructions.push({
            id: position.pozycja,
            name: name,
            value: position.sztuki - (position.progress / 11).toString().slice(0, 1),
          })
        }
      })
      constructionsInOrders.push({
        id: order,
        constructions: projects,
        projectString: projectString,
        notFinishConstructions: notFinishConstructions,
      })
    })
  } catch (ex) {
