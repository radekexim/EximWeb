...
export const TransportToPrint = React.forwardRef(({ data }, ref) => {
  const transport = data ? data : defaultData
                ...
                {transport.constructions.map((row) => (
                  <TableRow
                    ...
                    sx={{
                      ...
                      backgroundColor: row.notFinishConstructions.length === 0 && '#808080',
                    }}
                  >
                    <TableCell component='th' scope='row'>
                      {row.id}
                    </TableCell>
                    <TableCell align='left'>{row.projectString}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
            ...
            <Table size='small' aria-label='a dense table'>
              ...
              <TableBody>
                {transport.constructions
                  .filter((row) => row.notFinishConstructions.length > 0)
                  .map((row) => (
                    <>
                      <TableRow
                        ...
                        sx={{
                          '&:last-child td, &:last-child th': { border: 0 },
                          backgroundColor:
                            row.notFinishConstructions.length === 0 && '#808080',
                        }}
                      >
                      ...
                      {row.notFinishConstructions.map((position) => (
                        <TableRow key={uuid()}>
                          ...
                          <TableCell align='left'>{position.value + ' ' + position.name}</TableCell>
                        </TableRow>
                      ))}
                    </>
                  ))}
...
TransportToPrint.displayName = 'TransportToPrint'
