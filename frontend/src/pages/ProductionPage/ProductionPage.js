...
import { selectTotalSales, selectForecast, selectAllProductionOrders, selectAllProductionUnits, selectAllSalesUnits, fetchData, selectLastScans, selectAllSalesOrders } from './productionSlice';
...
export default function ProductionCharts(props) {
    ...
    const productionUnits = useSelector(selectAllProductionUnits);
    const salesUnits = useSelector(selectAllSalesUnits);
    const totalSales = useSelector(selectTotalSales);
    const productionOrders = useSelector(selectAllProductionOrders);
    const salesOrders = useSelector(selectAllSalesOrders);
    const forecast = useSelector(selectForecast);
    const lastScans = useSelector(selectLastScans);
    const goal = useSelector(state => state.production.goal);
    const month = useSelector(state => state.production.month);
    const chartStatus = useSelector(state => state.production.status)
    const error = useSelector(state => state.production.error)
    ...
    return chartStatus === 'loading' ? (
        <RefreshPage reload={() => dispatch(fetchData())} progress={chartStatus} />
                        ...
                        <TabPanel value="1">
                            ...
                            >
                                ...
                                <Grid
                                    ...
                                    xs={12}
                                >
                                    <TasksProgressCard progress={productionUnits[(productionUnits.length) - 1]} label={'Procent celu'} />
                                </Grid>
                                <Grid
                                    item
                                    xl={2}
                                ...
                                <Grid
                                    ...
                                    xs={12}
                                >
                                    <ColumnCharts seriesone={productionUnits.map((productionunit) => productionunit.units)} seriesonelabel={'Produkcja'} seriestwo={salesUnits.map((salesunit) => salesunit.units)} seriestwolabel={'Sprzedaż'} title={"Jednostki produkcyjne"} />
                                </Grid>
                                <Grid
                                    item
                                    lg={6}
                                    ...
                                    xs={12}
                                >
                                    <ColumnCharts seriesone={bonusUnits} title={"Jednostki premiowe"} legend={0} colorone={'#4caf50'} />
                                </Grid>
                            </Grid>
                        </TabPanel>
                        <TabPanel value="2">
                            ...
                            >
                                ...
                                >
                                    <Card sx={{ height: '100%', boxShadow: "0 8px 40px -12px rgba(0,0,0,0.3)" }}>
                                        ...
                                        <ConstructionTable data={constructionsCount} columns={['Konstrukcja', 'Sztuki', 'Jednostki']} title={'Konstrukcje wyprodukowane'} />
                                    </Card>
                                </Grid>
                                <Grid
                                    lg={4}
                                ...
                                >
                                    <Card sx={{ height: '100%', boxShadow: "0 8px 40px -12px rgba(0,0,0,0.3)" }}>
                                        <SelectList></SelectList>
                                        <ConstructionTable data={constructionsSalesCount} columns={['Konstrukcja', 'Sztuki', 'Jednostki']} title={'Konstrukcje sprzedane'} />
                                    </Card>
                                </Grid>
                            </Grid>
                        </TabPanel>
