...
export const fetchData = createAsyncThunk('production/fetchData', async () => {
  ...
  const newTotalSales =
    sumValueInObject(totalSalesVatRes.data) + sumValueInObject(totalSalesWithoutVat.data)
  const newForecast = forecastGoal(
    newProductionUnits[newProductionUnits.length - 1].units,
    initialState.goal,
  )
...
export const selectAll = (state) => state.production
