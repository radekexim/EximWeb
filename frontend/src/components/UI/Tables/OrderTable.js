...
export const OrderTable = (props) => {
    const columns = ['Pozycja', 'Konstrukcja', 'Sztuki', 'Uw', 'Sw', 'Powierzchnia', 'Waga', 'Metry', 'Kolor', 'Opis', 'Postęp', 'Obrazek']
                            ...
                            <TableBody>
                                {props.data.map((row) => (
                                    ...
                                    >
                                        ...
                                        <TableCell key={uuid()}>
                                            {row.uw ? (row.uw.toFixed(2)) : null}
                                        </TableCell>
                                        <TableCell key={uuid()}>
                                            {row.sw ? (row.sw.toFixed(2)) : null}
                                        </TableCell>
                                        <TableCell key={uuid()}>
                                            {row.powierzchnia}
                                        </TableCell>
                                        <TableCell key={uuid()}>
                                            {row.waga ? (row.waga.toFixed(2)) : null}
                                        </TableCell>
                                        <TableCell key={uuid()}>
                                            {row.metry}
                                        </TableCell>
                                        ...
                                        <TableCell key={uuid()}>
                                            <LinearProgressWithLabel value={Number(((row.progress / (row.sztuki * 12)) * 100).toFixed())} />
                                        </TableCell>
                                        <TableCell key={uuid()} >
